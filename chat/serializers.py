...
class ConversationCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Add creator to participants count
        total_participants = len(participant_ids) + 1
        
        # Set is_group based on participant count
        is_group = total_participants > 2
        
        # Set default name for groups if not provided
        name = validated_data.get('name')
        if is_group and not name:
            creator = self.context['request'].user
            if hasattr(creator, 'profile') and creator.profile.company_name:
                name = f"{creator.profile.company_name}'s Group"
            elif creator.full_name:
                name = f"{creator.full_name}'s Group"
            else:
                name = f"{creator.email}'s Group"
...
class GroupUpdateSerializer(serializers.ModelSerializer):
    """Serializer for updating group name"""
    class Meta:
        model = Conversation
        fields = ['name']
