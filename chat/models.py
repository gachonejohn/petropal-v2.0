...
class Conversation(models.Model):
    ...
    deleted_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name='deleted_groups')
    ...
    def get_creator_display_name(self):
        """Get display name for the group creator"""
        if hasattr(self.created_by, 'profile') and self.created_by.profile.company_name:
            return self.created_by.profile.company_name
        if self.created_by.full_name:
            return self.created_by.full_name
        return self.created_by.email  

    def soft_delete_group(self, deleted_by_user):
        """Safely delete a group conversation"""
        if not self.is_group:
            raise ValueError("Only applies to group conversations")
