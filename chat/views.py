...
class ConversationListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return Conversation.objects.filter(
            participants=self.request.user,
            is_deleted=False # Exclude soft deleted conversations
        ).exclude(
            user_deletions__user=self.request.user
        ).prefetch_related(
            'participants__profile',
...
class GroupManagementView(generics.GenericAPIView):
    """Base view for group management operations"""
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get_conversation(self, conversation_id):
        ...
        if conversation.created_by != self.request.user:
            raise PermissionError("Only the group creator can manage participants.")
...
def add_participants(request, conversation_id):
    ...
    try:
        ...
        serializer = GroupManagementSerializer(data=request.data)
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        
        participant_ids = serializer.validated_data['participant_ids']
        
        # Get all requested users (both existing and new)
        requested_users = Account.objects.filter(acc_id__in=participant_ids)
        
        # Check which users don't exist in the system
        found_ids = set(requested_users.values_list('acc_id', flat=True))
        not_found_ids = [pid for pid in participant_ids if pid not in [str(uid) for uid in found_ids]]
        
        # Get current participants
        current_participants = set(conversation.participants.values_list('acc_id', flat=True))
        ...
        for user in requested_users:
            if user.acc_id in current_participants:
                existing_participants.append({
                    'acc_id': user.acc_id,
                    'display_name': conversation.get_display_name(user),
                    'email': user.email
                })
            else:
        ...
        if new_participants:
            ...
            added_participants = [
                {
                    'acc_id': p.acc_id,
                    'display_name': conversation.get_display_name(p),
                    'email': p.email
        ...
        if added_participants:
            messages.append(f"Successfully added {len(added_participants)} new participant(s).")
        
        if existing_participants:
            messages.append(f"{len(existing_participants)} participant(s) were already in the group.")
        
        if not_found_ids:
            messages.append(f"{len(not_found_ids)} user(s) not found in system.")
        ...
        response_data = {
            ...
            "summary": {
                ...
                "added": len(added_participants),
                "already_existing": len(existing_participants),
...
def remove_participants(request, conversation_id):
    ...
    try:
        conversation = get_object_or_404(
            ...
            conversation_id=conversation_id,
            is_group=True,
            is_deleted=False  # Ensure group is not deleted
        )
        ...
        creator_id = str(conversation.created_by.acc_id)
        if creator_id in participant_ids:
            return Response(
        ...
        participants_to_remove = Account.objects.filter(
            acc_id__in=participant_ids,
            conversations=conversation
        )
        
        if not participants_to_remove.exists():
            return Response(
                {"error": "No valid participants found to remove."}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # Check if removal would leave only creator (less than 2 total participants)
        current_count = conversation.participants.count()
        remove_count = participants_to_remove.count()
        
        if current_count - remove_count < 2:
            return Response(
                {"error": "Cannot remove participants. Group must have at least 2 members."}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        # Remove participants
        with transaction.atomic():
            conversation.participants.remove(*participants_to_remove)
            
            # If group now has only 2 participants, convert to direct conversation
            if conversation.participants.count() == 2:
                conversation.is_group = False
                conversation.name = None  # Clear group name for direct conversations
                conversation.save(update_fields=['is_group', 'name'])
        
        response_data = {
            "message": f"Successfully removed {len(removed_participants)} participant(s).",
            "removed_participants": removed_participants
        }
        
        if not conversation.is_group:
            response_data["message"] += " Conversation converted to direct message."
...
def delete_group(request, conversation_id):
    """Delete a group conversation (soft delete)"""
    try:
        conversation = get_object_or_404(
            Conversation, 
            conversation_id=conversation_id,
            is_group=True,
            is_deleted=False  # Ensure group is not already deleted
        )
        ...
        # Confirmation is handled in the frontend
        
        # Get participants before deletion for response
        ...
        # Perform soft delete
        conversation.soft_delete_group(request.user)
        ...
        return Response({
            ...
            "deleted_by": conversation.get_creator_display_name(),
...
def update_group_name(request, conversation_id):
    ...
    try:
        ...
        if conversation.created_by != request.user:
            return Response(
                {"error": "Only the group creator can update the group name."}, 
                status=status.HTTP_403_FORBIDDEN
            )
        
        serializer = GroupUpdateSerializer(conversation, data=request.data, partial=True)
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        ...
        participants = [
            {
                ...
                'display_name': conversation.get_display_name(p),
                'email': p.email,
                'is_creator': p == conversation.created_by
            } for p in conversation.participants.all()
        ]
